...
export async function sendAdminPasswordResetEmail({ to, code, name }) {
  const from = process.env.EMAIL_FROM || "Ascendance <onboarding@resend.dev>";
  const subject = "Ascendance Admin Password Reset";
  const text = `Hello ${name || "Admin"},\n\nA request to reset your admin password for Ascendance was received.\n\nYour reset code is ${code}. It expires in 15 minutes.\n\nBrandZilla Tech Limited`;
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; color: #111;">
      <h2 style="color: #48006E; font-family: Georgia, serif;">Admin Password Reset</h2>
      <p>Hello ${name || "Admin"},</p>
      <p>We received a request to reset your admin password for Ascendance. Use the code below to complete the reset process.</p>
      <div style="margin: 32px 0; padding: 24px; background: rgba(72,0,110,0.05); text-align: center; border-radius: 8px;">
        <div style="font-size: 32px; font-weight: bold; letter-spacing: 8px; color: #48006E;">${code}</div>
      </div>
      <p style="color: #666; font-size: 14px;">This code expires in 15 minutes. If you did not request a password reset, please ignore this email.</p>
      <hr style="border: none; border-top: 1px solid #eee; margin: 32px 0;" />
      <p style="color: #999; font-size: 12px; text-align: center;">BrandZilla Technologies</p>
  ...
  if (!process.env.RESEND_API_KEY) {
    console.log(`[Ascendance admin password reset] ${to}: ${code}`);
    return { ok: true, provider: "console" };
  }
  ...
  if (!response.ok) {
    const errorText = await response.text();
    throw new Error(`Admin password reset email failed: ${errorText}`);
  }
