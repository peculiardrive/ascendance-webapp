...
export async function readState() {
  try {
    ...
    if (error.code === "ENOENT") {
      // Fallback: try to read the committed data/next-state.json from process.cwd() if on Vercel
      try {
        const fallbackPath = join(process.cwd(), "data", "next-state.json");
        const raw = await readFile(fallbackPath, "utf8");
        const saved = JSON.parse(raw);
        // Save it to the temp directory so subsequent requests can read it quickly
...
export function normalizeState(state) {
  ...
  const books = savedBooks.map((book) => {
    ...
    return canonicalBook ? { ...book, cover: canonicalBook.cover, usdPrice: canonicalBook.usdPrice, blurb: canonicalBook.blurb, summary: canonicalBook.summary } : book;
  });
...
export function publicBook(book, state, userId, isAdmin = false) {
  return {
    ...book,
    sections: (book.sections || []).map((section) => ({
      ...section,
      chapters: (section.chapters || []).map((chapter) => {
        const locked = !isAdmin && !hasChapterAccess(state, userId, book, chapter);
        return {
          ...chapter,
          locked,
